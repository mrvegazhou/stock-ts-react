...
export default function RoleTreeComponent(props: Props): JSX.Element {
	...
	const dataToJson = useCallback((one, data) => {
		...
		if (!one) {
			...
			kids = data.filter((item: RoleLevel) => item.parent === one.uuid);
		}
	...
	const onOk = useCallback(() => {
		...
		const res = props.data.filter((item) => {
			return nowKeys.includes(`${item.uuid}`);
		});
		// 返回选中的keys和选中的具体数据
	...
	return (
		<Modal
			...
			onCancel={onClose}
		>
			<Tree checkable selectable={false} checkedKeys={nowKeys} onCheck={onCheck} treeData={sourceData} />
		</Modal>
	);
}
