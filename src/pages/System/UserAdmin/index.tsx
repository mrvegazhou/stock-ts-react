...
// 类型声明
// ==================
import { TableRecordData, Page, operateType, ModalType, SearchInfo, UserBasicInfoParam, Res, RoleTreeInfo } from "./index.type";
...
function UserAdminContainer(): JSX.Element {
	...
	// 函数 - 查询当前页面所需列表数据
	async function onGetData(page: { page_num: number; page_size: number }): Promise<void> {
	...
	// 删除某一条数据
	const onDel = async (uuid: number): Promise<void> => {
		setLoading(true);
		try {
			const res = await dispatch.sys.delUser({ uuid });
			if (res && res.code === 200) {
	...
	// 表格页码改变
	const onTablePageChange = (page_num: number, page_size: number): void => {
		onGetData({ page_num, page_size });
	};
	...
	const onRoleOk = async (keys: string[]): Promise<void> => {
		...
		const params = {
			admin_user_id: modal.nowData.uuid,
			role_ids: keys.map((item) => Number(item)),
		};
		setRole({
	...
	const tableColumns = [
		...
		},
		{
			...
			render: (v: null, record: TableRecordData) => {
				const controls = [];
				const u = userinfo.userBasicInfo || { uuid: -1 };
				...
				<Form form={form} initialValues={{ formStatus: 1 }}>
					...
					<Form.Item
						...
						rules={[
							...
							{
								pattern: new RegExp(
									"^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*?[^A-Za-z0-9])(?=.*[@$!%*#?&])[A-Za-z\\d@$!#%*?&]{6,18}$",
									"g"
								),
								message: "密码需要满足:1.全英文;2.最少一个大写字母;3.最少一个英文特殊字符;4.最少一个数字;5.范围在6-18位;",
							},
						]}
					>
