...
import { Page, TableRecordData, operateType, ModalType, PowerTreeInfo, SearchInfo, RoleParam, Role, Res } from "./index.type";
...
function RoleAdminContainer() {
	...
	// 删除某一条数据
	const onDel = async (role_id: number) => {
		setLoading(true);
		try {
			const res = await dispatch.sys.delRole({ role_id });
			if (res && res.code === 200) {
	...
	const onAllotPowerClick = (record: TableRecordData) => {
		const menus = record.menu_powers.map((item) => item.menu_id); // 需默认选中的菜单项ID
		// 需默认选中的权限ID
		const powers = record.menu_powers.reduce((v1, v2) => [...v1, ...v2.powers], []);
