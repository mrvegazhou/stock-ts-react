	...
	const onBtnDtoChange = useCallback(
		(e, id, record) => {
			...
			if (e.target.checked) {
				// 选中
				old.push(id);
				treeCheckedTemp = Array.from(new Set([record.uuid, ...treeChecked]));
			} else {
				// 取消选中
				old.splice(old.indexOf(id), 1);
				// 判断当前这一行的权限中是否还有被选中的，如果全都没有选中，那当前菜单也要取消选中
				const tempMap = record.powers.map((item: Power) => item.uuid);
	...
	const tableRowSelection = useMemo(() => {
		return {
			...
			onSelect: (record: TableData, selected: boolean): void => {
				const t = props.data.find((item) => item.uuid === record.uuid);
				if (selected) {
					// 选中，连带其权限全部勾选
					if (t && Array.isArray(t.powers)) {
						const temp = Array.from(new Set([...t.powers.map((item) => item.uuid), ...btnDtoChecked]));
						setBtnDtoChecked(temp);
					}
				} else {
					// 取消选中，连带其权限全部取消勾选
					if (t && Array.isArray(t.powers)) {
						const mapTemp = t.powers.map((item) => item.uuid);
				...
				if (selected) {
					...
					setBtnDtoChecked(
						props.data.reduce((v1: any, v2: any) => {
							return [...v1, ...v2.powers.map((k: any) => k.uuid)];
						}, [])
					);
				} else {
					setBtnDtoChecked([]);
					...
					if (value) {
						return value.map((item: Power, index: number) => {
							return (
								<Checkbox
									key={index}
									checked={dtoIsChecked(item.uuid)}
									onChange={(e): void => onBtnDtoChange(e, item.uuid, record)}
								>
									{item.title}
								</Checkbox>
							);
