...
// 组件
// ==================
import { IconsData } from "@/utils/icon";
...
function MenuAdminContainer() {
	...
	const onTreeSelect = useCallback((keys, e) => {
		...
		if (e.selected) {
			// 选中
			treeSelect = { title: e.node.title, id: e.node.uuid };
		}
	...
	const getNameByParentId = (id: number | null) => {
		const p = data.find((item) => item.uuid === id);
	...
	const onDel = async (record: TableRecordData) => {
		const params = { menu_id: record.uuid };
